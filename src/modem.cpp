...
void defaultEEPROM()
{
    ...
    setEEPROM("irata.online:8005", speedDialAddresses[0], 50);
    setEEPROM("cyberserv.org:8005", speedDialAddresses[1], 50);
...
void welcome()
{
    Serial.println("PLATO SDRIVE & WIFI BY @JEFFPIEP FOR ESP8266 BASED ON ...");
    Serial.println("github.com/whizzosoftware/SIO2Arduino  GPLv1");
    Serial.println("github.com/RolandJuno/esp8266_modem    GPLv3");
    Serial.println("");
}
...
void displayHelp()
{
    ...
    //  yield();
    //  Serial.println("PET MCTERM TR.: ATPETN (N=0,1)");
    //  yield();
...
void handleRoot()
{
    ...
    page.concat("\nIP ADDRESS.: ");
    page.concat(ipToString(WiFi.localIP()));
    page.concat("\nGATEWAY....: ");
    page.concat(ipToString(WiFi.gatewayIP()));
    yield();

    page.concat("\nSUBNET MASK: ");
    page.concat(ipToString(WiFi.subnetMask()));
    yield();
    page.concat("\nSERVER PORT: ");
    page.concat(tcpServerPort);
    page.concat("\nCALL STATUS: ");
    if (callConnected)
    {
...
void checkConnection()
{
    ...
    if ((!tcpClient.connected()) && callConnected == true)
    {
        cmdMode = true;
        sendResult(R_NOCARRIER);
        connectTime = 0;
 ...
 */
    ...
    webServer.begin();
    mdns.begin("plato", WiFi.localIP()); // i don't think the IP is needed
}
