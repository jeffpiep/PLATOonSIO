...
uint32_t DiskImage::getSectorSize()
{
  return m_sectorSize;
}
 ...
 */
SectorDataInfo *DiskImage::getSectorData(uint32_t sector, byte *data)
{
  ...
  case TYPE_XEX:
  {
    if (sector < 4)
    {
      uint32_t ix = (sector - 1) * m_sectorSize;
      for (unsigned int i = 0; i < m_sectorSize; i++)
      {
    ...
    else
    {
      ...
      m_fileRef->seek((sector - 4) * m_sectorSize, SeekSet); //SPIFSS
    }
  }
  break;
#endif
  ...
  case TYPE_PRO:
  {
    ...
    m_fileRef->seek(m_headerSize + ((sector - 1) * (m_sectorSize + sizeof(PROSectorHeader))), SeekSet); //SPIFFS
    ...
    else
    {
      ...
      if (m_usePhantoms && m_proSectorHeader.totalPhantoms > 0 && m_phantomFlip)
      {
        ...
        m_fileRef->seek(m_headerSize + (((720 + m_proSectorHeader.phantom1) - 1) * (m_sectorSize + sizeof(PROSectorHeader))) + sizeof(PROSectorHeader), SeekSet); //SPIFFS
      }
    }
  ...
  case TYPE_ATX:
  {
    ...
    if (ix > -1)
    {
      ...
      m_fileRef->seek(m_sectorHeaders[ix].fileIndex, SeekSet); //SPIFFS
    ...
    else
    {
      ...
      // m_fileRef->seekSet(0);
      m_fileRef->seek(0, SeekSet); //SPIFFS
      m_sectorInfo.error = true;
  ...
  default:
    ...
    m_fileRef->seek(m_headerSize + ((sector - 1) * m_sectorSize), SeekSet); //SPIFSS
    break;
  }
 ...
 */
uint32_t DiskImage::writeSectorData(uint32_t sector, byte *data, uint32_t len)
{
  if (!m_readOnly)
  {
    // seek to proper offset in file
    uint32_t offset = m_headerSize + ((sector - 1) * m_sectorSize);
    // m_fileRef->seekSet(offset);
    m_fileRef->seek(offset, SeekSet); // SPIFFS
...
boolean DiskImage::format(File *file, int density)
{
  if (!m_readOnly)
  {
    ...
    uint32_t length = FORMAT_SS_SD_40;

    // make sure we're at beginning of file
    // file->seekSet(0);
    file->seek(0, SeekSet); //SPIFFS
...
boolean DiskImage::loadFile(File *file)
{
...
#ifdef DEBUG1
    Serial1.print("Loaded ATR with sector size ");
    Serial1.print(atrHeader->secSize, DEC);
    Serial1.println(". ");
    Serial1.flush();
#endif
  ...
  {
    LOG_MSG_CR("Header is not a match.");
  }
  ...
  if (header[0] == 'A' && header[1] == 'T' && header[2] == '8' && header[3] == 'X')
  {
    ...
    for (int i = 0; i < 40; i++)
    {
      ...
      file->seek(fileIndex + l2, SeekSet); //SPIFFS
  ...
  String tempName = file->name();     //SPIFFS
  #ifdef DEBUG1
  LOGGING_UART.println(tempName);
  #endif
  tempName.toCharArray(filename, 13); //SPIFFS
