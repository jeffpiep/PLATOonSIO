    ...
    switch (m_cmdPinState) {
      ...
      case STATE_WAIT_CMD_START:
        if (digitalRead(m_cmdPin) == LOW) {
          ...
          //LOGGING_UART.println("Command Start!");
          //LOGGING_UART.flush();
        }
        break;
      case STATE_READ_CMD:
        ...
        // otherwise, check for command read timeout
          ...
          m_cmdPinState = STATE_WAIT_CMD_START;
          LOG_MSG_CR("Command Read TIMEOUT");
        }
        break;
      case STATE_READ_DATAFRAME:
        ...
        if (millis() - m_startTimeoutInterval > READ_FRAME_TIMEOUT) {
          m_cmdPinState = STATE_WAIT_CMD_START;
          LOG_MSG_CR("Dataframe read TIMEOUT");
        }
        break;
      case STATE_WAIT_CMD_END:
        if (digitalRead(m_cmdPin) == HIGH) {
          m_cmdPinState = STATE_WAIT_CMD_START;
          // LOG_MSG_CR("Waiting for Command ...");
        }
        break;      
    }
}
...
void SIOChannel::cmdGetSector(int deviceId) {
  ...
  if (p != NULL) {
    ...
    for (unsigned int i=0; i < SD_SECTOR_SIZE + 1; i++) {
...
void SIOChannel::cmdFormat(int deviceId, int density) {
  ...
  if (m_driveAccess->formatFunc(deviceId, density)) {
    ...
    m_stream->write(0xFF);
    m_stream->write(0xFF);
    for (unsigned int i=0; i < SD_SECTOR_SIZE - 3; i++) {
      m_stream->write((byte)0x00);
    }
    m_stream->write(0xFF);
    m_stream->write(0xFF);
...
#endif  
#ifdef DEBUG1
      m_sdriveHandler.printCmdName(m_cmdFrame.command);
#endif

}

uint32_t SIOChannel::getCommandSector() {
  return (uint32_t)(m_cmdFrame.aux2 << 8) + (m_cmdFrame.aux1 & 0xff);
}
